...
class HuggingfaceDataset(object):
    ...
    def __init__(self, config, tokenizer, text_processor):
        ...
        # Set up tmpfs data directory
        os.makedirs(self.config.tmpfs_path, exist_ok=True)

        # Determine shard index based on IP address
        self.shard_index = self._get_shard_index()
        self.hostname = socket.gethostname()
        print(f"Initializing dataset with shard index: {self.shard_index} on server: {self.hostname}")

        # Load or create the dataset
        self._load_or_create_dataset()

    def _get_shard_index(self):
        # Get the last octet of the IP address
        ip = socket.gethostbyname(socket.gethostname())
        last_octet = int(ip.split('.')[-1])
        return last_octet % self.config.num_shards

    def _load_or_create_dataset(self):
        dataset_file = os.path.join(self.config.tmpfs_path, f'dataset_shard_{self.shard_index}.jsonl')
        
        if os.path.exists(dataset_file):
            print(f"Loading existing dataset shard from {dataset_file} on server: {self.hostname}")
            self._dataset = self._load_dataset_from_file(dataset_file)
        else:
            print(f"Creating new dataset shard and saving to {dataset_file} on server: {self.hostname}")
            self._create_and_save_dataset_shard(dataset_file)

    def _load_dataset_from_file(self, file_path):
        with open(file_path, 'r') as f:
            return [json.loads(line) for line in f]

    def _create_and_save_dataset_shard(self, file_path):
        ...
        shard_size = len(full_dataset) // self.config.num_shards
        start_idx = self.shard_index * shard_size
        end_idx = start_idx + shard_size if self.shard_index < self.config.num_shards - 1 else len(full_dataset)

        with open(file_path, 'w') as f:
            for example in full_dataset[start_idx:end_idx]:
                f.write(json.dumps(example) + '\n')

        self._dataset = self._load_dataset_from_file(file_path)
    ...
    def __iter__(self):
        ...
        for index, example in enumerate(self._dataset):
            tokens, loss_masks = self._text_processor(example)
            token_buffer.extend(tokens)
            ...
            while len(token_buffer) > chunk_size + 1:
                ...
                if self.config.always_start_with_bos:
                    batch['input_tokens'][:, 0] = self._tokenizer.bos_token_id
                yield batch, metrics
